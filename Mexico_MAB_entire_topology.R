...
# Set the working directory to the parent directory of the app.R file
...
for (hl4 in local_nodes) {
  ...
  mg1_packets <- sapply(1:arms, function(i) {
    path_v <- shortest_paths(g, from = V(g)[hl4],to = V(g)[regional_nodes[i]],output = c("vpath"))$vpath[[1]]
...
# Create a matrix of TRUE/FALSE values
best_paths_v2_matrix <- matrix(FALSE, nrow = length(regional_nodes), ncol = length(best_paths_v2))
# Set the corresponding position in the matrix to TRUE for each number
...
# Create a matrix of TRUE/FALSE values
best_paths_real_matrix <- matrix(FALSE, nrow = length(regional_nodes), ncol = length(best_paths_real))
# Set the corresponding position in the matrix to TRUE for each number
