...
# such as Epsilon-Greedy, Upper Confidence Bound (UCB), and SoftMax. 
...
# penalties equal to a average delay*(-1) scenario.
...
for (test in 1:tests){
  ...
  for (i in 1:n_trials) {
    ...
    max_upper_bound = -Inf # step 3 bit to initialize the max upper bound at zero
    for (arm in 1:arms) {
      if (counts[arm] == 0) {
        ucb_values[arm] <- Inf  # If an arm hasn't been selected, prioritize its selection
      } else {
        average_reward <- rewards[arm] / counts[arm]
        ucb_values[arm] <- average_reward + sqrt(3/2*log(i)/counts[arm])
      }
      if (ucb_values[arm] > max_upper_bound) { # step 3 bit to readjust max upper bound on each round
        max_upper_bound = ucb_values[arm]
        chosen_arm = arm
      }
    }
...
prob_opt_path_selected_ucb1 <- prob_opt_path_selected/100
...
prob_opt_path_selected_Softmax <- prob_opt_path_selected/100
