...
file_name_v2 <- "input_files/Metro_topology_synthetic.xlsx"
nodes_info <- read_excel(file_name_v2, sheet = 1)
links_info <- read_excel(file_name_v2, sheet = 2)

national_nodes <- which(nodes_info$node_code == "HL2" | nodes_info$node_code == "HL3")
...
# Node names
V(g)$name[local_nodes] <- paste0("Node_", local_nodes)
V(g)$name[national_nodes] <- paste0("Node_", V(g)$name[national_nodes])
#V(g)$name[regional_nodes] <- paste0("Node_", V(g)$name[regional_nodes])
...
E(g)$color <- "gray"
plot(g, edge.arrow.size=.5, vertex.label = V(g)$name, edge.curved=.5, layout=l)
...
n_trial_convergence_v3 <- c()
n_trial_convergence_v4 <- c()
n_trial_convergence_v5 <- c()
n_trial_convergence_v6 <- c()
n_trial_convergence_v7 <- c()
n_trial_convergence_v8 <- c()


matching_percent_v1 <- c()
matching_percent_v2 <- c()
matching_percent_v3 <- c()
matching_percent_v4 <- c()
matching_percent_v5 <- c()
matching_percent_v6 <- c()
matching_percent_v7 <- c()
matching_percent_v8 <- c()
...
convergence_thresholds <- c(15,10,8,6,4,3,2,1) #c(10, 3) #c(20,17,15,12,10:1) #
...
for (hl4 in local_nodes) {
  ...
  n_trial_convergence_test_v1 <- c()
  n_trial_convergence_test_v2 <- c()
  n_trial_convergence_test_v3 <- c()
  n_trial_convergence_test_v4 <- c()
  n_trial_convergence_test_v5 <- c()
  n_trial_convergence_test_v6 <- c()
  n_trial_convergence_test_v7 <- c()
  n_trial_convergence_test_v8 <- c()
  
  
  
  best_paths_test_matching_v1 <- c()
  best_paths_test_matching_v2 <- c()
  best_paths_test_matching_v3 <- c()
  best_paths_test_matching_v4 <- c()
  best_paths_test_matching_v5 <- c()
  best_paths_test_matching_v6 <- c()
  best_paths_test_matching_v7 <- c()
  best_paths_test_matching_v8 <- c()
  
  
  for (test in 1:tests){
    best_paths_v1[test] <- MAB_algorithm_av_delays(mg1_packets, arms, conv_num, convergence_thresholds[1])
    n_trial_convergence_test_v1[test] <- n_trial_convergence
    best_paths_v2[test] <- MAB_algorithm_av_delays(mg1_packets, arms, conv_num, convergence_thresholds[2])
    n_trial_convergence_test_v2[test] <- n_trial_convergence
    
    
    best_paths_v3[test] <- MAB_algorithm_av_delays(mg1_packets, arms, conv_num, convergence_thresholds[3])
    n_trial_convergence_test_v3[test] <- n_trial_convergence
    best_paths_v4[test] <- MAB_algorithm_av_delays(mg1_packets, arms, conv_num, convergence_thresholds[4])
    n_trial_convergence_test_v4[test] <- n_trial_convergence
    
    best_paths_v5[test] <- MAB_algorithm_av_delays(mg1_packets, arms, conv_num, convergence_thresholds[5])
    n_trial_convergence_test_v5[test] <- n_trial_convergence
    best_paths_v6[test] <- MAB_algorithm_av_delays(mg1_packets, arms, conv_num, convergence_thresholds[6])
    n_trial_convergence_test_v6[test] <- n_trial_convergence
    best_paths_v7[test] <- MAB_algorithm_av_delays(mg1_packets, arms, conv_num, convergence_thresholds[7])
    n_trial_convergence_test_v7[test] <- n_trial_convergence
    
    best_paths_v8[test] <- MAB_algorithm_av_delays(mg1_packets, arms, conv_num, convergence_thresholds[8])
    n_trial_convergence_test_v8[test] <- n_trial_convergence
    
  }
  
  best_paths_test_matching_v1 <- ifelse(best_paths_v1 == best_paths_real[n], 1,0)
  best_paths_test_matching_v2 <- ifelse(best_paths_v2 == best_paths_real[n], 1,0)
  best_paths_test_matching_v3 <- ifelse(best_paths_v3 == best_paths_real[n], 1,0)
  best_paths_test_matching_v4 <- ifelse(best_paths_v4 == best_paths_real[n], 1,0)
  best_paths_test_matching_v5 <- ifelse(best_paths_v5 == best_paths_real[n], 1,0)
  best_paths_test_matching_v6 <- ifelse(best_paths_v6 == best_paths_real[n], 1,0)
  best_paths_test_matching_v7 <- ifelse(best_paths_v7 == best_paths_real[n], 1,0)
  best_paths_test_matching_v8 <- ifelse(best_paths_v8 == best_paths_real[n], 1,0)
  
  
  n_trial_convergence_v1[n] <- mean(n_trial_convergence_test_v1)
  n_trial_convergence_v2[n] <- mean(n_trial_convergence_test_v2)
  n_trial_convergence_v3[n] <- mean(n_trial_convergence_test_v3)
  n_trial_convergence_v4[n] <- mean(n_trial_convergence_test_v4)
  n_trial_convergence_v5[n] <- mean(n_trial_convergence_test_v5)
  n_trial_convergence_v6[n] <- mean(n_trial_convergence_test_v6)
  n_trial_convergence_v7[n] <- mean(n_trial_convergence_test_v7)
  n_trial_convergence_v8[n] <- mean(n_trial_convergence_test_v8)
  
  matching_percent_v1[n] <- mean(best_paths_test_matching_v1)
  matching_percent_v2[n] <- mean(best_paths_test_matching_v2)
  matching_percent_v3[n] <- mean(best_paths_test_matching_v3)
  matching_percent_v4[n] <- mean(best_paths_test_matching_v4)
  matching_percent_v5[n] <- mean(best_paths_test_matching_v5)
  matching_percent_v6[n] <- mean(best_paths_test_matching_v6)
  matching_percent_v7[n] <- mean(best_paths_test_matching_v7)
  matching_percent_v8[n] <- mean(best_paths_test_matching_v8)
...
cat("N trials for", convergence_thresholds[3], "%: ", mean(n_trial_convergence_v3))
cat("N trials for", convergence_thresholds[4], "%: ", mean(n_trial_convergence_v4))
cat("N trials for", convergence_thresholds[5], "%: ", mean(n_trial_convergence_v5))
cat("N trials for", convergence_thresholds[6], "%: ", mean(n_trial_convergence_v6))
cat("N trials for", convergence_thresholds[7], "%: ", mean(n_trial_convergence_v7))
cat("N trials for", convergence_thresholds[8], "%: ", mean(n_trial_convergence_v8))
# cat("N trials for", convergence_thresholds[9], "%: ", mean(n_trial_convergence_v9))
# cat("N trials for", convergence_thresholds[10], "%: ", mean(n_trial_convergence_v10))
# cat("N trials for", convergence_thresholds[11], "%: ", mean(n_trial_convergence_v11))
# cat("N trials for", convergence_thresholds[12], "%: ", mean(n_trial_convergence_v12))
# cat("N trials for", convergence_thresholds[13], "%: ", mean(n_trial_convergence_v13))
# cat("N trials for", convergence_thresholds[14], "%: ", mean(n_trial_convergence_v14))
...
# frames = subset(df, best_paths_v2)
# frames = data.frame(HL2s = frames$HL2s  , HL4s = frames$HL4s )
# 
# frames_destination = data.frame(HL4s = 1:length(best_paths_real), HL2s = best_paths_real )
# 
# df_v3 <- rbind(subset(df, df$best_paths_v1),
#                subset(df, df$best_paths_v2)
# )
# 
# df2    <- df_v3[duplicated(interaction(df_v3$HL4s , df_v3$HL2s)),]
# df2    <- df2[rep(seq(nrow(df_v3)), each = 3),]
# df2$x1 <- df2$HL4s
# df2$y1 <- df2$HL2s
...
# 
# df3    <- df_v3[duplicated(interaction(df_v3$HL4s , df_v3$HL2s)),]
# df3    <- df3[rep(seq(nrow(df_v3)), each = 3),]
# df3$x1 <- df3$HL4s
# df3$y1 <- df3$HL2s
# df3$x1 <- df3$x1 + c(-0.5, 0.5, 0.5)*(-1)
# df3$y1 <- df3$y1 + c(-0.5, -0.5, 0.5)*(-1)
# df3$z  <- rep(seq(nrow(df3)/3), each = 3)
# df$overlap <- df$best_paths_v1 & df$best_paths_v2
# 
# df4 <- subset(df, best_paths_v1 & !df$overlap)
# df4 <- data.frame( HL4s = df4$HL4s, HL2s = df4$HL2s)
# 
# df5 <- subset(df, best_paths_v2 & !df$overlap)
# df5 <- data.frame( HL4s = df5$HL4s, HL2s = df5$HL2s)
# 
# 
# frames_v1 = subset(df, best_paths_v1)
# frames_v1 = data.frame(HL2s = frames_v1$HL2s  , HL4s = frames_v1$HL4s )
# frames_v2 = subset(df, best_paths_v2)
# frames_v2 = data.frame(HL2s = frames_v2$HL2s  , HL4s = frames_v2$HL4s )
# 
# 
...
#                       breaks = seq(0, max(df$delay * 1e6), by = 5)) +
#   geom_tile(data = frames_v1, fill = "brown1") +
#   geom_tile(data = frames_v2, fill = "royalblue1") +
#   geom_polygon(data = df2, aes(x = x1, y = y1, group = z), fill = "brown1", size = 0) +
#   geom_polygon(data = df3, aes(x = x1, y = y1, group = z), fill = "royalblue1", size = 0) +
#   geom_tile(data = frames_destination, color = "#74C476", linewidth = 2, fill = NA, alpha = 0.3) +
#   
...
#   labs(x = "HL4 IDs", y = "HL2 IDs") +  # Add axis labels here
#   theme(
...
cat("Average Number of convergence threshold ", convergence_thresholds[1],"% average error:", 100 - mean(matching_percent_v1)*100, "%\n")
cat("Average Number of convergence threshold ", convergence_thresholds[2],"% average error:", 100 - mean(matching_percent_v2)*100, "%\n")
cat("Average Number of convergence threshold ", convergence_thresholds[3],"% average error:", 100 - mean(matching_percent_v3)*100, "%\n")
cat("Average Number of convergence threshold ", convergence_thresholds[4],"% average error:", 100 - mean(matching_percent_v4)*100, "%\n")
cat("Average Number of convergence threshold ", convergence_thresholds[5],"% average error:", 100 - mean(matching_percent_v5)*100, "%\n")
cat("Average Number of convergence threshold ", convergence_thresholds[6],"% average error:", 100 - mean(matching_percent_v6)*100, "%\n")
cat("Average Number of convergence threshold ", convergence_thresholds[7],"% average error:", 100 - mean(matching_percent_v7)*100, "%\n")
cat("Average Number of convergence threshold ", convergence_thresholds[8],"% average error:", 100 - mean(matching_percent_v8)*100, "%\n")
